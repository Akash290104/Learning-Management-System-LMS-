...
const markCurrentLectureAsViewed = async (req, res) => {
  try {
    ...
    console.log("Error marking current lecture as viewed", error);
...
const getCurrentCourseProgress = async (req, res) => {
  try {
    ...
    if (!isCurrentCoursePurchasedByCurrentUser) {
      console.log("You need to purchase this course to access it");
    ...
    console.log("Error fetching current course progress", error);
    return res.status(500).json({
      success: false,
...
const resetCoursesProgress = async (req, res) => {
  try {
    ...
    console.log("Error reseting course progress", error);
    return res.status(500).json({
      success: false,
